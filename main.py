...
def _console(tag: str, msg: str, level: str = "info"):
    line = f"[Captcha][{tag}] {msg}"
    try:
...
class CaptchaPlugin(Star):
        ...
        try:
            if os.path.exists(self.config_path):
                with open(self.config_path, "r", encoding="utf-8") as f:
                    user = json.load(f)
                default.update(user)
                _console("CONFIG", f"配置加载成功: {self.config_path}")
        except Exception as e:
            _console("CONFIG", f"配置加载失败: {e}", level="error")
        return default

    def _dbg(self, tag: str, msg: str):
        if self.cfg.get("debug_print", True):
            _console(tag, msg)
    ...
    async def _handle_captcha(self, event: AstrMessageEvent, msg_str: str):
        t0 = time.time()
        _console("CAPTCHA", "============== 🎯 验证码处理开始 ==============")
        try:
            m = re.search(r"第(\d+)个表情", msg_str)
            target_num = int(m.group(1)) if m else 1
            self._dbg("CAPTCHA", f"目标编号 = {target_num}")

            img_m = re.search(r"!\[.*?\]\((https://qqbot\.ugcimg\.cn/.*?)\)", msg_str)
            if not img_m:
                _console("CAPTCHA", "未找到验证码图片链接！", level="error")
                return
            ...
            if not (buttons_data and msg_seq and bot_appid):
                _console("CAPTCHA",
                         f"关键参数缺失: msg_seq={msg_seq!r}, bot_appid={bot_appid!r}, "
                         f"buttons={len(buttons_data)}", level="error")
                return
            ...
            if not target_btn:
                _console("MATCH", f"AI 答案【{ai_answer}】不在选项内，盲点第一个", level="warning")
                target_btn = buttons_data[0]

            _console("CLICK", f"准备点击按钮：【{target_btn['label']}】")

            client = self._get_onebot_client(event)
            if client is None:
                _console("CLICK", "未获取到 aiocqhttp 底层 client，无法点击。", level="error")
                return
            ...
            try:
                ...
                _console("CLICK", f"call_action 失败: {e}", level="error")

        except Exception as e:
            _console("CAPTCHA", f"全局异常: {e}\n{traceback.format_exc()}", level="error")
        finally:
            _console("CAPTCHA", f"============== 🎯 验证码处理结束（耗时 {time.time()-t0:.2f}s）==============")
